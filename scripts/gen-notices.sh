...
# .Module.Dir is empty until the module is in the local cache. A module graph
# with nothing in it makes this exit non-zero ("no module dependencies to
# download"), which is not a failure here — see the empty-graph branch below —
# so its status is deliberately not fatal.
...
# a blank line.
# `|| true` guards the empty case rather than hiding errors: with pipefail set,
# a `grep -v` that filters away every line exits 1, and for a stdlib-only module
# every line is filtered away. Without this the pipeline kills the script before
# the empty-graph branch below can report the honest answer.
go list -deps -f '{{with .Module}}{{printf "%s\t%s\t%s\t%s" .Path .Version .Dir $.Dir}}{{end}}' "$PKG" \
    | grep -v '^[[:space:]]*$' \
    | grep -v "^${main_module}	" \
    | sort -u > "$work/packages" || true

# Zero third-party packages is this repository's actual state: go.mod declares no
# requires and there is no go.sum, because the proxy is written against the
# standard library alone. That is worth stating in a file somebody can read
# rather than leaving as an absence — and worth re-deriving on every CI run, so
# the claim cannot quietly become false the first time a dependency is added.
if [ ! -s "$work/packages" ]; then
    generated="$work/notices.md"
    cat > "$generated" <<'NODEPS'
# Third-party notices
...
NODEPS
    if [ "$check" = true ]; then
        ...
        echo "$OUT is up to date (no third-party modules)"
        exit 0
    fi
    mv "$generated" "$OUT"
    echo "wrote $OUT (no third-party modules)"
    exit 0
fi
...
The `mysql-old-password-proxy` binary is statically linked, so the compiled code of every
